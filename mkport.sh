...
then
	...
	ssh $node sudo ovs-vsctl add-port br-int $tap_name -- set interface $tap_name type=internal -- set interface $tap_name external_ids=attached_mac=\\\"$mac\\\",iface-id=$port_uuid,iface-status=active
fi
exit $?
